...
void create_edges(int const &, int const &, vector<pair<int, int>>&, std::ifstream&);
...
int main(int argc, char* argv[]) {
    ...
    gD.dfsRank = std::vector<int>(gD.n, -1);
    gD.dfsRank.shrink_to_fit();
    gD.parent = std::vector<int>(gD.n, -1);
    gD.parent.shrink_to_fit();
    gD.nDescendants = std::vector<int>(gD.n, 1);
    gD.nDescendants.shrink_to_fit();
    gD.ear = std::vector<pair<int, int>>(gD.n, std::pair<int, int>(-1, -1));
    ...
    std::chrono::duration<double, std::nano> time_elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(time_end - time_start);
    std::cout << "time elapsed is: " << std::setprecision(4) << time_elapsed.count()/1000 << "μs\n";
    ...
    //     std::cout << "Vertex " << i << " | DFS " << gD.dfsRank[i] << " | Parent " << gD.parent[i] << " | nDescendants " << gD.nDescendants[i] << " | ear " << gD.ear[i].first << ',' << gD.ear[i].second << "\n";
    // }
...
void genCS(int const & starting_vertex, graphDataHolder & gD) {
    //std::stack<int, std::vector<int>> the_stack;
...
void create_edges(int const & num_v, int const & num_e, vector<pair<int, int>>& edges, std::ifstream& istream) {
    edges.reserve(num_e);
